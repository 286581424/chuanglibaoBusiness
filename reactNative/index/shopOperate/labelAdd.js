...
export default class labelAdd extends Component {
    constructor(props) {
    super(props);
    this.state = {
      title: '添加属性',
  ...
  addSure(){
    ...
    const { params } = this.props.navigation.state;
    if (this.state.title == '添加属性') {
      let str = '?token=' + this.state.token[1]
      let JsonObj = {property_name:this.state.skuName,property_value:JSON.stringify(this.state.skuList)}
      NetUtils.postJson('physical/goods/property/add', JsonObj ,str, (result) => {
          Alert.alert('提示','添加成功!',[{text:'确定',onPress:() => navigate('labelSetting',{key:'success'})}])
      });
    }else{
      let str = '?token=' + this.state.token[1]
      let JsonObj = {id:params.id,property_name:this.state.skuName,property_value:JSON.stringify(this.state.skuList)}
      NetUtils.postJson('physical/goods/property/update', JsonObj ,str, (result) => {
          Alert.alert('提示','修改成功!',[{text:'确定',onPress:() => navigate('labelSetting',{key:'success'})}])
      });
    }
  }
  ...
  loadSkuDetails(id){
    setTimeout(() => {
        ...
        NetUtils.get('physical/goods/property/detail', params, (result) => {
            console.log(result)
            this.setState({skuName:result.property_name})
            let param_value = result.property_value
  ...
  delete(){
    ...
    NetUtils.get('physical/goods/property/delete', str, (result) => {
        Alert.alert('提示','删除成功!',[{text:'确定',onPress:() => navigate('labelSetting',{key:'success'})}])
    });
  }
            ...
            <View style={styles.container}>
                ...
                <View style={{backgroundColor:'white',width:ScreenUtils.scaleSize(750),height:ScreenUtils.scaleSize(this.state.statusBarHeight),backgroundColor:'white'}}>
                   ...
                   <View style={{backgroundColor:'white',width:ScreenUtils.scaleSize(750),height:ScreenUtils.scaleSize(100),flexDirection:'row',alignItems:'center'}}>
                      <Text style={{color:'black',left:ScreenUtils.scaleSize(30),width:ScreenUtils.scaleSize(150),fontSize:ScreenUtils.setSpText(8)}}>*属性参数</Text>
