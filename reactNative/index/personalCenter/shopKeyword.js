...
export default class shopKeyword extends Component {
    constructor(props) {
    super(props);
    this.state = {
      title: '关键字',
  ...
  componentDidMount() {
    ...
    this.loadBusinessID();
    this.loadMobile();
    if (this.props.navigation.state.params.shopKeyword != null) {
        this.setState({numberArr:[this.props.navigation.state.params.shopKeyword]});
    }
  }
    ...
    _saveNumBtnPress(navigate,params){
     if(this.state.numberArr != null){
         if (params.key == 'shopSeting') {
          let shopInfoStr = {id:this.state.businessID,mobile:this.state.mobile,address_key:this.state.numberArr[0]};
           NetUtils.postJson('business/updateBusiness',shopInfoStr,'',(result) => {
              navigate(params.key,{shopKeyword:this.state.numberArr});
           });
         }else{
            navigate(params.key,{shopKeyword:this.state.numberArr});
         }
